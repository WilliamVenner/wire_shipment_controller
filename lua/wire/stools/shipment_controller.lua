...
if CLIENT then
	...
	language.Add("tool.wire_shipment_controller.inputs.separatepricemarkup", "[NUMBER] Adds extra money to the separate price output")
	...
	language.Add("tool.wire_shipment_controller.outputs.separateprice", "[NUMBER] Price of item when sold separately, if not separate the output will default to price / size")
	language.Add("tool.wire_shipment_controller.outputs.nameandprice", "[STRING] Combination of name and price")
	language.Add("tool.wire_shipment_controller.outputs.nameandseparateprice", "[NUMBER] Combination of name and separate price")
...
if SERVER then
	...
	local inputs = { "Dispense", "Price Markup", "Separate Price Markup", "Currency", "Out Of Stock Message" }
	local outputs = { "Quantity", "Size", "Price", "Name", "Category", "Type", "Model", "Separate", "Separate Price", "Name And Price", "Name And Separate Price", "Shipment" }
